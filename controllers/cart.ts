...
import { _addToCart, _clearCart, _decreaseItem, _findCart, _increaseItem, _removeProductFromCart } from "../services/cart";
...
export const removeProductFromCart = async (req: Request, res: Response) => {
    try {
        ...
        if (cart_name) {
            const cart = await _removeProductFromCart((req as any).userid, (barcode as string), Number(cart_name));
            res.status(200).json(cart);
        }
