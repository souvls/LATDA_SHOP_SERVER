...
export const _updateProduct = async (barcode: string, updateFields: ProductUpdateAttributes) => {
...
export const _updateIMGProduct = async (barcode: string, filename: any) => {
    const update = await Product.update({ img_name: filename }, { where: { barcode } });
    return update;
}
